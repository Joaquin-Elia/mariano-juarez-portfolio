export interface ProjectMedia {
  type: "image" | "video";
  src: string;
  caption?: string;
  fullWidth?: boolean;
  margin?: boolean;
  customClass?: string;
}

export interface MediaRow {
  type: "row";
  layout: "grid" | "overlap";
  columns?: number;
  items: ProjectMedia[];
}

export type ProjectContent = ProjectMedia | MediaRow;

export interface Project {
  slug: string;
  title: string;
  year: string;
  description: string;
  media: ProjectContent[];
  nextProjectSlug: string;
}

const projects: Project[] = [
  {
    slug: "virus-2000",
    title: "VIRUS 2000",
    year: "(2025)",
    description:
      "Proyecto experimental que plantea un nuevo disco de Virus situado en los años 2000. A partir de esta premisa, se desarrolla un lenguaje visual inspirado en el futurismo Y2K, combinando estética retro y digital con tipografías sintéticas, brillos y elementos tecnológicos. La propuesta construye una identidad contemporánea donde lo analógico y lo digital conviven en una misma narrativa, integrando inteligencia artificial como recurso en la generación de imágenes.",
    media: [
      {
        type: "row",
        layout: "grid",
        columns: 2,
        items: [
          { type: "image", src: "v1777420922/Tapa-1_wj4pby" },
          { type: "image", src: "v1777420922/Contratapa-Coverplastico_vrqv7v" },
        ],
      },
      {
        type: "image",
        src: "Inside_zcoqwm",
        fullWidth: true,
        margin: true,
      },
      {
        type: "row",
        layout: "grid",
        columns: 2,
        items: [
          { type: "image", src: "insert-a_lki6rg_fee6ef" },
          { type: "image", src: "Insert-B_lzriqk" },
        ],
      },
      {
        type: "row",
        layout: "grid",
        columns: 2,
        items: [
          {
            type: "image",
            src: "Vinilo-lado-a_rxgnc7",
            customClass: "md:mt-[-7%]",
          },
          {
            type: "image",
            src: "Vinilo-lado-b_y8ikuh",
            customClass: "md:mt-[-1%]",
          },
        ],
      },
      {
        type: "image",
        src: "Virus2000logo_vpvwuh",
        customClass: "md:max-w-5xl md:mx-auto",
        fullWidth: true,
        margin: true,
      },
    ],
    nextProjectSlug: "flyemigrates",
  },
  {
    slug: "flyemigrates",
    title: "FLYEMIGRATES",
    year: "(2021)",
    description:
      'Desarrollo de identidad integral para marca de streetwear independiente. El proyecto abarca desde el diseño textil y el desarrollo de sistemas de estampas hasta la creación de un ecosistema de packaging utilitario. La propuesta visual se inspira en el lenguaje de la logística global y los sistemas de transporte, utilizando una paleta acromática y tipografías industriales para construir una estética "ready-to-ship". Flyemigrate propone un uniforme para la migración urbana, donde la funcionalidad del diseño y la calidad del material son los protagonistas.',
    media: [
      {
        type: "row",
        layout: "overlap",
        items: [
          {
            type: "image",
            src: "Reme-3_hzbtll",
            customClass: "md:w-[85vw] md:mt-[-25%] md:max-w-none z-0",
          },
          {
            type: "image",
            src: "Reme-1_db09bt",
            customClass:
              "md:w-[85vw] md:max-w-none md:ml-[-25%] md:mt-[15%] rotate-[-10deg] z-[1]",
          },
          {
            type: "image",
            src: "Reme-2_q03mck",
            customClass:
              "md:w-[85vw] md:max-w-none md:ml-[-25%] md:mt-[-25%] z-[2]",
          },
        ],
      },
      {
        type: "row",
        layout: "grid",
        columns: 3,
        items: [
          { type: "image", src: "modelo2_ma18ku" },
          { type: "image", src: "modelo1_scamr0" },
          { type: "image", src: "modelo3_qngbci" },
        ],
      },
      {
        type: "row",
        layout: "grid",
        columns: 3,
        items: [
          { type: "video", src: "4_k2rq2v", customClass: "md:max-h-96" },
          { type: "video", src: "7_odtewx" },
          { type: "video", src: "6_mvypvl", customClass: "md:max-h-96" },
        ],
      },
      {
        type: "row",
        layout: "grid",
        columns: 3,
        items: [
          { type: "video", src: "3_fmnwd7" },
          { type: "video", src: "2_ndtjhk" },
          { type: "video", src: "1_cvuoky" },
        ],
      },
      {
        type: "row",
        layout: "grid",
        columns: 3,
        items: [
          { type: "image", src: "Close-Up_Print_Detail_grxebl" },
          { type: "image", src: "Logo_Detail_Close-Up_eqrihe" },
          { type: "image", src: "Close-Up_Detail_v1_qeedes" },
        ],
      },
      {
        type: "row",
        layout: "overlap",
        items: [
          {
            type: "image",
            src: "parcel_bag_front_gbfyhh",
            customClass:
              "z-[2] md:w-[70vw] rotate-[-5deg] md:ml-[5%] md:-top-36",
          },
          {
            type: "image",
            src: "parcel_bag_front2_oxps70", // Reusing same if second not found
            customClass: "md:w-[70vw] rotate-[-2deg] md:ml-[-10%] md:-top-28",
          },
        ],
      },
    ],
    nextProjectSlug: "tumbandando-el-club",
  },
  {
    slug: "tumbandando-el-club",
    title: "TUMBANDANDO EL CLUB",
    year: "(2021)",
    description:
      'Este proyecto desarrolla el ecosistema visual para el relanzamiento simbólico de "Tumbando el Club", el himno fundacional del trap argentino. La propuesta no se limita a la promoción de un single; construye una mitología digital que traduce la energía, el "calor" de la calle y la sinergia del colectivo en un lenguaje visual agresivo, técnico y altamente coleccionable.',
    media: [
      {
        type: "video",
        src: "Parte_Sony_julio_hrxyps",
        fullWidth: true,
      },
      {
        type: "row",
        layout: "overlap",
        items: [
          {
            type: "image",
            src: "Card-Lucho-Case_w5msxc",
            customClass: "md:min-w-[10vw] md:mt-[-30%] z-[1] md:ml-[3%]",
          },
          {
            type: "image",
            src: "Card-Duki-Case_ahysg2",
            customClass: "md:min-w-[16vw] z-[1] md:mt-[-15%] md:ml-[-2%]",
          },
          {
            type: "image",
            src: "Card-Khea-Case_dgklwc",
            customClass: "md:min-w-[13vw] md:ml-[-4%] md:mt-[-2%] z-[2]",
          },
          {
            type: "image",
            src: "Card-ysy-Case_cg89xp",
            customClass: "md:min-w-[17vw] md:ml-[-11%] md:mt-[-20%] z-0",
          },
          {
            type: "image",
            src: "Card-Cazzu-Case_pt7pxc",
            customClass: "md:min-w-[13vw] md:ml-[-4%] md:mt-[-3%] z-2",
          },

          {
            type: "image",
            src: "Card-Obi-Case_o7qmcg",
            customClass:
              "md:min-w-[12vw] md:ml-[-8%] md:mt-[-18%] z-1 rotate-[-6deg]",
          },
          {
            type: "image",
            src: "Card-Neo-Case_oa9bsj",
            customClass:
              "md:min-w-[12vw] md:ml-[-14%] md:mt-[13%] z-0 rotate-[2deg]",
          },
        ],
      },
      {
        type: "row",
        layout: "grid",
        columns: 2,
        items: [
          {
            type: "video",
            src: "v1777420922/secuencia_fdi73y",
            customClass: "md:mt-[-10%]",
          },
          { type: "image", src: "duks_rksbbu", customClass: "md:mt-[-10%]" },
        ],
      },
      {
        type: "row",
        layout: "grid",
        columns: 2,
        items: [
          { type: "image", src: "1_ah98gy" },
          { type: "image", src: "luchs2_ko7166" },
        ],
      },
    ],
    nextProjectSlug: "cerro-catedral",
  },
  {
    slug: "cerro-catedral",
    title: "CERRO CATEDRAL",
    year: "(2026)",
    description:
      'Este fotolibro es un ejercicio de nostalgia técnica. Utiliza la degradación estética del VHS para documentar la inmensidad del Cerro Catedral, Bariloche. Al capturar fotogramas, el proyecto subvierte el uso de la tecnología: emplea un formato "obsoleto" y ruidoso para retratar un paisaje eterno y monumental.',
    media: [
      { type: "image", src: "Cover-fotolibro_eeboxw", fullWidth: true },
      { type: "image", src: "4_dts13c", fullWidth: true,  },
      { type: "image", src: "3_khpqyg", fullWidth: true, customClass: "md:mb-16"},
      {
        type: "row",
        layout: "grid",
        columns: 4,
        items: [
          { type: "image", src: "estampa_lhfzzq" },
          { type: "image", src: "estampa-copia2_wldkiy" },
          { type: "image", src: "estampa3_k4sn88" },
          {
            type: "image",
            src: "passticketsnow_monqud",
            customClass: "max-w-72 md:max-w-[30vw] lg:max-w-[12vw] mx-auto",
          },
        ],
      },
      { type: "image", src: "2_iwfv91", fullWidth: true, customClass: "md:mt-26" },
      { type: "image", src: "1_p1d01n", fullWidth: true },
      // {
      //   type: "row",
      //   layout: "overlap",
      //   items: [
      //     { type: "image", src: "BEANIE_e0cqja" },
      //     {
      //       type: "image",
      //       src: "BEANIE2_zmmnns",
      //       customClass: "md:-mt-24 md:-ml-20",
      //     },
      //   ],
      // },
    ],
    nextProjectSlug: "virus-2000",
  },
];

export default projects;
